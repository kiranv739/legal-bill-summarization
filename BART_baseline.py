...
# === Load model ===
model_path = "facebook/bart-large-cnn"  # Replace with your actual fine-tuned path
...
def summarize_chunk(chunk_tensor):
    try:
        input_text = tokenizer.decode(chunk_tensor, skip_special_tokens=True)
        encoded = tokenizer(input_text, return_tensors="pt", truncation=True, max_length=1024).to(device)
...
def process_bill(pdf_path):
    ...
    full_summary = "\n\n".join(summaries)
    out_path = Path("summaries_bart") / f"{pdf_path.stem}.txt"
